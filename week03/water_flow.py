...
def pressure_gain_from_water_height(water_column_height):
    ...
    pressure = (water_density * earth_acceleration_of_gravity * water_column_height) / 1000

    return pressure

def pressure_loss_from_pipe(pipe_diameter, pipe_length, friction_factor, fluid_velocity):

    pressure_lost_pipe = (-friction_factor * pipe_length * water_density * (fluid_velocity ** 2)) / (2000 * pipe_diameter)

    return pressure_lost_pipe

def pressure_loss_from_fittings(fluid_velocity, quantity_fittings):

    pressure_lost_fittings = (-0.04 * water_density * (fluid_velocity ** 2) * quantity_fittings) / 2000

    return pressure_lost_fittings
...
def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number, smaller_diameter):
    ...
    pressure_loss_pipe_reduction = (-constant_k * water_density * (fluid_velocity ** 2)) / 2000
